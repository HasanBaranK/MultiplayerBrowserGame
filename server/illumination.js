...
function generatelightSource(x, y, name, range, power, lightSources) {
    ...
    let index = lightSources.push(lightSource);
    lightSource.index = index - 1
    return lightSource;
}
...
function calculateGeneralLight(time, generalLightAmount) {
    if (time.hour < 6 || time.hour > 20) {
        generalLightAmount = 15;
    } else if (time.hour < 11) {
        //6 7 8 9 10
        generalLightAmount = 15 + ((time.hour - 6) * 60 + time.minute) * (1 / 3)
    } else if (time.hour >= 11 && time.hour <= 15) {
        generalLightAmount = 100
    } else if (time.hour > 15) {
        //16 17 18 19 20
        generalLightAmount = 115 - (((time.hour - 16) * 60 + time.minute) * (1 / 3))
    }
...
function calculateLighting(lightSources, lightMap, collisionMap, generalLightAmount, players,gridSize) {
    ...
    for (let lightSource in lightSources) {
        ...
        if (lightSource !== null) {
            ...
            if (lightSource.type == "Point" && shouldCalculate) {
                for (let i = 0; i < lightSource.range; i += gridSize) {
                    ...
                    for (let k = 0; k < lightSource.range; k += gridSize) {
                        let lightAmount = 0;
